...
function createPathFilter(globPaths, basePath) {
  ...
  for (const rawGlobPath of globPaths) {
    const match = new Minimatch(rawGlobPath, {
      dot: true,
      magicalBraces: true,
      windowsPathsNoEscape: true
    });

    for (let i = 0; i < match.set.length; i++) {
      /** Path segments containing strings, globstar symbols, and RegExps. */
      const set = match.set[i];
      /** The current glob pattern (uses slashes `/` even for Windows). */
      const globPath = match.globSet[i];
      debug('processing glob pattern: %s', globPath);

      // get path segment strings until a glob or regexp is matched
      const globIndex = set.findIndex(entry => typeof entry !== 'string');
      const isGlob = globIndex > -1;
      if (isGlob) {
        debug('found glob or regexp at segment index: %o', globIndex);
      }
      const segments = /** @type {string[]} */ (
        set.slice(0, isGlob ? globIndex : set.length)
      );

      // match `cleanPath` (path without the glob part) and its subdirectories
      // path segments should be joined first to preserve empty strings
      const cleanPath = path.resolve(basePath, segments.join(path.sep));
      ...
      const absPath = path.resolve(basePath, globPath);
