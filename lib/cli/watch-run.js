...
/**
 * Extracts out paths without the glob segments, directory paths,
 * and paths for matching from the provided glob paths.
...
function createPathFilter(globPaths, basePath) {
  /**
   * The resulting object for path filtering.
   * @type {PathFilter}
   */
  const res = {
  ...
  for (const globPath of globPaths) {
    ...
    for (const pattern of globInstance.patterns) {
      ...
      // for matching directories under the `cleanPath`
 ...
 * @param {boolean} [checkIsParent] Treats the provided path as a match if it is a valid parent directory from the list of paths.
 * @returns {boolean} Determines if there was a match.
 * @ignore
 * @private
 */
function matchPattern(filePath, pattern, checkIsParent) {
...
/**
 * Performs a `map.set(key, value)` but will also clear the map
 * if it reaches the specified maximum size.
 * @template TKey The map key.
 * @template TValue The map value.
 * @param {Map<TKey, TValue>} map The map to use.
 * @param {TKey} key The key to use.
 * @param {TValue} value The value to set.
 * @ignore
 * @private
 */
function mapSet(map, key, value) {
  // clear map whenever the limit is reached
...
function createPathMatcher(allowed, ignored, basePath) {
  ...
  const matcher = {
    allow(filePath) {
      ...
      mapSet(allowCache, filePath, allow);
      return allow;
    },
    ...
    ignore(filePath, stats) {
      ...
      // for some reason, chokidar paths for the ignore match function
      // uses slashes `/` even for windows
      // resolve to ensure correct absolute path
      ...
      if (matchPattern(filePath, ignored.match)) {
        ignore = true;
      } else if (matcher.allow(filePath)) {
        // check allowed paths via `matcher.allow()` to also cache the result
        ignore = false;
        ...
        ignore = !matchPattern(filePath, allowed.dir, true);
      } else {
        ignore = true;
      }

      mapSet(ignoreCache, filePath, ignore);
      return ignore;
    }
  };
